...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("stepcline")
    ...
    **Note:** To ensure that the `stepcline` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:
