...
export function isHumanEdit(e: vscode.TextDocumentChangeEvent): boolean {
	...
	if (e.contentChanges.length > 1) {
		console.log("🚀🤖 human edit content", e.contentChanges)
		return false
	}
	...
	if (e.contentChanges.length === 1) {
		...
		if (totalChangedChars > 100) {
			console.log("🚀🤖 human edit total change chars", totalChangedChars)
			return false
		}
		...
		if (newlineCount > 2 && change.text.length > 50) {
			console.log("🚀🤖 newlineCount", newlineCount, "change.text.length", change.text.length)
		...
		if (change.text.length > 30) {
			...
			if (hasCodePatterns && hasIndentation) {
				console.log("🚀🤖 key words", totalChangedChars)
				return true // Structured code is likely AI-generated or copy-pasted
			}
		}
	}
